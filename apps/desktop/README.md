...
- `local_changes` 记录本地 create / update / status / delete 变更，供 WebDAV 同步 push 阶段使用。
...
- `TaskRepository.recordSyncRun()` / `listRecentSyncRuns()` 维护同步运行历史：记录每次手动 WebDAV 同步的成功/失败、开始/结束时间、message 与 cursor，作为设置页可见性的数据来源。
...
- 设置页在 WebDAV“立即同步”成功或失败后会刷新本地数据库统计、同步状态、待同步队列与同步历史，让 runner 写回的 cursor / lastError / pending count 即时可见。
...
- 同步报告卡片显示：上传 ops 数、本地标记已同步数、拉取 ops 数、应用任务数、删除任务数、结果 message。
- `WebdavSettingsCard` 在同步完成（成功或失败）后会向父级 emit `sync-done` 事件，设置页据此重新拉取本地数据库 / 同步状态 / 待同步变更 / 同步历史。
...
- 在同一台机器创建若干任务，确认上传 ops 数与“待同步”归零。
- 在第二台机器（或同一机器换 SQLite 数据库副本）配同一组凭据，点击“立即同步”，确认能拉到对端任务。
