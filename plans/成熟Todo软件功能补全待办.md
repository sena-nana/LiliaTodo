...
- [x] 日历页已升级为日 / 周 / 月视图，支持日期浏览、拖拽改期、排期建议、容量负载和冲突检测。
- [x] 全局任务视图已有所有、四象限和时间线三种只读 / 操作入口。
- [x] WebDAV 同步主路径已具备手动触发、凭据保存、push / pull、冲突基线和运行结果展示。
- [x] 当前仓库已具备 Agent runtime 状态、Agent 收件箱、Codex runner 桥、确认队列、审计撤销、真实系统通知、重复任务、全局搜索筛选和低频自动触发链路。
...
- [x] P0-04 新增 Agent 收件箱
  - 目标：给用户一个统一入口处理 Agent 建议、确认、执行结果和撤销。
  - 当前状态：侧边栏已有 Agent 收件箱入口；页面以紧凑队列展示 runtime 状态、待确认操作、工具集、执行批次、最近处理和 runtime 事件。
...
- [x] P0-05 定义首批 Todo Agent 工具集
  - 目标：让 Agent 可以围绕任务管理核心形成可执行闭环。
  - 当前状态：已在前端 Agent 领域层定义首批 Todo Agent 工具集，包含稳定 action type、输入 schema、中文摘要、风险等级和 dry-run 结果。
...
- [x] P0-06 实现确认队列和写入保护
  ...
  - 当前状态：已新增 `agent_pending_actions`，Agent 建议先进入待确认操作；用户确认后由执行器调用 `TaskRepository`，拒绝会记录状态和原因。
...
- [x] P0-07 实现审计记录和批次撤销
  - 目标：让主动执行闭环可追踪、可恢复。
  - 当前状态：已新增 `agent_audit_records` 记录 action、before / after、来源 envelope、批次和执行状态；可逆批次支持从 Agent 收件箱撤销。
...
- [x] P0-08 接入低频关键事件触发
  ...
  - 当前状态：任务创建 / 更新、逾期、提醒到期和每日首次启动已接入真实自动扫描链路；自动触发设置关闭时不会扫描 runtime 或写入确认队列；runtime 未运行时只记录中文诊断，不构建快照、不扫描、不写入建议。
  - 实现要点：在任务创建 / 更新后节流触发；应用每日首次启动触发一次全局检查；逾期和提醒到期触发建议，不直接改期；触发事件进入 runtime envelope。
  - 验收标准：同一任务短时间多次变更会合并触发；用户可在设置中关闭自动触发；自动触发只生成建议，不跳过确认队列；确认队列执行使用原始仓库方法，避免 Agent 写入回流再次触发自动扫描。

- [x] P0-09 本地任务上下文快照
  - 目标：为 Agent 提供足够但克制的 Todo 上下文。
  - 当前状态：已新增本地任务上下文快照构建器，统一读取任务、清单、分类和任务时间 / 提醒 / 标签 / 估时 / 父子关系，并限制数量和文本长度。
...
- [x] P1-01 全局搜索与筛选：支持按标题、备注、标签、清单、分类、状态、优先级、时间范围和提醒状态检索任务。
- [x] P1-02 重复任务：支持每日、每周、每月、自定义间隔和完成后生成下一次实例。
- [x] P1-03 真实系统通知：提醒到期时通过 Tauri / 系统通知提示，支持稍后提醒、关闭和打开任务。
- [x] P1-04 批量操作：支持多选任务后批量完成、改期、移动清单、设置标签、删除，并与 Agent 写入保护共用风险提示。
- [x] P1-05 完成 / 归档管理：提供已完成、已归档、最近删除或等价恢复入口，避免任务完成后不可追踪。
- [x] P1-06 清单 / 分类管理入口：在 UI 中补齐创建、重命名、排序、归档清单和创建、重命名、删除分类的完整入口。
- [x] P1-07 拖拽排序与重排：收件箱、清单、分类、子任务和检查项支持稳定拖拽排序。
- [x] P1-08 键盘快捷键：补齐快速添加、打开详情、完成、删除、搜索、跳转今日 / 收件箱 / Agent 收件箱等常用快捷键。
- [x] P1-09 错误和空状态一致性：统一页面加载失败、空列表、同步失败、Agent 失败和权限不足的中文表达与重试入口。

## P2 计划与智能体验

- [x] P2-01 日历周 / 月 / 日视图：从未来 7 天只读列表升级为可浏览、可拖拽改期的成熟日历。
- [x] P2-02 自动排期建议：结合截止时间、估时、优先级、开始时间和每日容量生成排期建议；当前已在日历页生成可应用建议，用户点击确认后才写入任务。
- [x] P2-03 容量与负载分析：按天 / 周展示估时总量、逾期风险、资源占用和过载提示。
- [x] P2-04 冲突检测：检测时间重叠、父任务未拆解、提醒过晚、截止时间早于开始时间等问题。
- [x] P2-05 智能视图保存：允许保存自定义筛选视图，例如“本周高优先级”“无计划任务”“等待确认”。
- [x] P2-06 任务模板：支持把常用任务结构保存为模板，包括检查项、默认标签、估时和提醒。
- [x] P2-07 导入导出：支持 Markdown / JSON / CSV 导入导出，便于迁移和备份。
- [x] P2-08 Agent 复盘报告：按日 / 周生成完成情况、延期原因、计划偏差和下周建议。
